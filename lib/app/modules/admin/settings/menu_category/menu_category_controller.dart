...
class MenuCategoryController extends GetxController {
  ...
  Future<void> createMenuCategory(String name, String fkMenuId, String description, File? menuFile) async {
  ...
  try {
    ...
    if (response.statusCode == 201) {
      ...
      responseMessage.value = 'Failed to create menu. Status: ${response.statusCode}';
...
int? menuCategoryId;
  ...
  selectedMenuCategory(val) {
    var selectedMenuCategory = menuItemCategory.value.firstWhere(
      ...
      orElse: () => Menu(id: -1,  name: val, description: '', coverImage: '', active: false, deleted: false, createdAt: DateTime.now(), updatedAt: DateTime.now(), deletedAt: DateTime.now()),
    );
